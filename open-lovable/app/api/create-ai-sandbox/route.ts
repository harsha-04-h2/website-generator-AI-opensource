...
async function createSandboxInternal() {
  ...
  try {
    ...
    console.log("[create-ai-sandbox] OIDC token exists:", !!oidcToken);
    ...
    if (oidcToken) {
      if (process.env.VERCEL_TEAM_ID && process.env.VERCEL_PROJECT_ID) {
        ...
        sandboxConfig.token = oidcToken;
      } else if (process.env.VERCEL_OIDC_TOKEN || oidcToken.startsWith('vcp_')) {
        console.log('[create-ai-sandbox] Using OIDC/VCP token authentication');
        sandboxConfig.oidcToken = oidcToken;
        // Ensure the x-vercel-oidc-token header is present for Vercel SDK
